...
The verified dev account ID `484632959006` is the only deployment value in the workflow-level `env` block and is used for the wrong-account check. The workflow reads the dev role ARN, region, and SSM parameter name from GitHub repository variables. Optional staging and production promotion reads the corresponding role ARN and account-ID variables only when that target is selected. Shell steps read repository variables directly, and Terraform steps export them only inside the running shell when Terragrunt requires process variables. GitHub Environments are not used.
