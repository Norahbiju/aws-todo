...
Authorisation is layered: repository write access permits dispatch, global workflow permissions control token scope, branch rules protect reviewed code, AWS OIDC trust restricts tokens to this repository's `main` branch, and IAM policies restrict API actions. GitHub Environments are not used. Deployment configuration is stored in repository variables; configure the corresponding account ID and role ARN before selecting each target.

The verified dev account ID `484632959006` is the only deployment value in each workflow-level `env` block; staging and production account IDs remain repository variables. All targets retain explicit wrong-account checks. No job or step has an `env` block. Terraform shell steps export repository values during execution because Terragrunt reads its backend and module inputs from process variables. These are not GitHub Environments.

Actions use explicit release versions. Multi-line scripts use strict Bash mode. Required checks do not continue after failure. Deployment concurrency is serialised by target and never cancels an in-progress mutation.
...
Verify each workflow's global `permissions`, action versions, explicit working directories, main-branch check, account-check step, and summary. Protect `.github/workflows/**` and `infra/**` with CODEOWNERS and repository rulesets.
