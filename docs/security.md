...
Identity controls use short-lived GitHub OIDC sessions, account-ID checks, provider allowlisting, protected environments, branch rules, and least-privilege roles. No AWS key, PAT, OIDC token, or Terraform state is committed or transferred between jobs. ECS execution and task roles are distinct and minimal.
...
Protect main; require pull-request and CODEOWNERS review; require application and Terraform checks; restrict workflow changes; require staging/prod environment reviewers; prevent self-review; restrict prod OIDC to its environment; retain GitHub/AWS audit logs; and use SCPs or permission boundaries for additional guardrails.
